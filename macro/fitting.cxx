...
void fitting::DrawGraph( TH2* h2d, bool doFit, int nbins, double* binlow, float xlow_fit, float xhigh_fit, TString outputtxtname,int phi,int eta,int charge,int side){
  double sag,sagerr;
  ofstream fout;
  ...
  TH1* frame=gPad->DrawFrame(0, 0, 0.25, 0.2);
...
//    fout << "A/pT+B(1/pT)^2 " << g->GetName() << " A= " <<  testFunc->GetParameter(0) << " B= " << testFunc->GetParameter(1) << " chi2= " << testFunc->GetChisquare() << endl;
    //fout << eta << " " << phi << " " << testFunc->GetParameter(0) << " " << testFunc->GetParameter(1) << " " << charge << " " << side << " " << testFunc->GetChisquare() << endl;
    fout << eta << " " << phi << " " << testFunc->GetParameter(0) << " " << testFunc->GetParameter(1) << " " << charge << " " << side << endl;
  }
  leg->Draw();
  //TString pname="pictureAlphaBeta/";
  TString pname="pictureL2AlphaBetaTGC1000/";
  pname+=g->GetName();
  pname+=".root";
...
TGraphErrors* fitting::MakeGraph(TH2* h2d, int nbins, double* binlow){
  ...
  for( int ibin=0; ibin<nbins; ibin++ ){
    ...
    if( fabs(ptcenter) > 0.26 || fabs(ptcenter) < 0.001 ){
      continue;
    }
...
//  double mean=0;
  ...
  ///
  //TF1 *f1 = new TF1("f1","gaus",mean-1.2*sigma, mean+1.2*sigma);
  ...
  ///
  //TF1 *f2 = new TF1("f2","gaus",mean-1.2*sigma, mean+1.2*sigma);
  //f1->SetParameter(1,mean);
  //h1d->Fit("f2");
  ...
  sigma=gausFunc->GetParameter(2);
  ///
  //TF1 *f3 = new TF1("f3","gaus",mean-1.2*sigma, mean+1.2*sigma);
  //f3->SetParameter(1,mean);
  //h1d->Fit("f3");
  //mean=f3->GetParameter(1);
  //sigma=f3->GetParameter(2);
  h1d->Fit( gausFunc, "", "", mean-1.2*sigma, mean+1.2*sigma );
  mean=gausFunc->GetParameter(1);
  sigma=gausFunc->GetParameter(2);
  ///
  //TF1 *f4 = new TF1("f4","gaus",mean-1.2*sigma, mean+1.2*sigma);
  //f4->SetParameter(1,mean);
  //h1d->Fit("f4");
  h1d->Fit( gausFunc, "", "", mean-1.2*sigma, mean+1.2*sigma );

  //return make_pair( f4->GetParameter(1),
		    //f4->GetParError(1) );
