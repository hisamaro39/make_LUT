#include "macro/fittingLargeSpecial.h"
#include "TFile.h"
...
void fitting::DrawGraph( TH2* h2d, bool doFit, int nbins, double* binlow, float xlow_fit, float xhigh_fit, TString outputtxtname,int qeta,int phi,int etabin,int phibin){
  ...
  if(doFit){
    if( qeta==0 ) {
      testFunc->SetParLimits(0,0,0.001);
      testFunc->SetParLimits(1,0,100);
      testFunc->SetParLimits(2,0,100);
      testFunc->SetLineColor(4);
      //g->Fit(testFunc, "", "", xlow_fit, xhigh_fit);
      g->Fit(testFunc, "", "", 0, 400000);
    }
    /*else {
      testFunc2->SetParLimits(0,0,0.3);
      testFunc2->SetParLimits(1,0,100);
  ...
  leg->Draw();
  TString pname="pictureBarrelRadius/data16_LS_shift_pol2/";
  pname+=g->GetName();
  pname+=".root";
